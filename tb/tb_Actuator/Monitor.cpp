...

{
	...
	fout.open("tb/tb_Actuator/output.txt");
	fresult.open("tb/tb_Actuator/result.txt");	
	fmean.open("tb/tb_Actuator/mean.txt");	
...
void Monitor::checkOutput(float_pid output) {
	...
	if(fout.is_open()) {
		...
		if (fout >> exp_output) {
	    	...
	    	if( (output-exp_output)>ERROR_ACTUATOR || (output-exp_output)<(-1*ERROR_ACTUATOR) ) {
	    		cout << "Should be ";
	    		printf("%f", exp_output);
	    		error_count++;
	    	}
