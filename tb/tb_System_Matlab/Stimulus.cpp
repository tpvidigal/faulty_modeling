...
Stimulus::Stimulus(sc_module_name inst) : sc_module(inst),
	...
	initSetpoint("InitSetpoint")

{
	cout << "Constructing Stimul\t" << name() << endl;

	fset.open("tb/tb_System_Matlab/setpoint.txt");
...
void Stimulus::doStimulus() {
	...
	while(true) {
		...
		if(fset.is_open()) {
				...
				if (fset.good())
					setpoint = (float_pid) bin2double(set_bin);
				else
					break;
		...
		trans->set_data_ptr( reinterpret_cast<unsigned char*>(&setpoint) );
		...
		// Transport call
		cout << "\n\n[" << sc_time_stamp() << "]   \t" << flush;
		cout << "Sending setpoint (" << setpoint << ")\t" << flush;
		initSetpoint->b_transport( *trans, delay );
